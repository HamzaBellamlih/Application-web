...
class Article(models.Model):
    ...
    prix_restant = models.DecimalField(
        max_digits=10, decimal_places=2, default=Decimal("0.00")
    )
    ...
    def __str__(self):
        return (
            f"{self.id} le {self.date_creation} et modifié le {self.date_modification} {self.type} {self.nom} ({self.epaisseur} mm) "
...
class Mesure(models.Model):
    article = models.ForeignKey(
        Article, related_name="mesures", on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return (
            f"ID {self.id} → {self.longueur} x {self.largeur} ({self.nombre_de_fois}x, "
...
class Planche(models.Model):
    article = models.ForeignKey(
        "article", on_delete=models.CASCADE, related_name="planches"
    )
...
class Piece(models.Model):
    planche = models.ForeignKey(
        Planche, on_delete=models.CASCADE, related_name="pieces"
    )
    nom = models.CharField(max_length=100)
